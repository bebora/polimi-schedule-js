const strings = {
    ...
    "it": {
        "text": {
            ...
            "gcalendarText": "L'app può inserire gli eventi su Google Calendar direttamente per semplificare il più possibile l'importazione, senza dover scaricare nessun file.<br>Per fare ciò, richiede esplicitamente il consenso dell'utente per collegarsi all'account Google. In particolare, l'app ha bisogno di leggere i nomi dei calendari esistenti per poi proporli se l'utente vuole importare gli eventi in un calendario esistente. L'utente può voler creare un nuovo calendario per gli eventi, quindi l'app ha bisogno anche dei permessi in scrittura sul calendario Google. Sono necessari quindi permessi completi sul calendario, ma l'app non usa ora né userà mai i permessi per scopi differenti da quelli di pubblica utilità qui descritti.<br>È possibile vedere la privacy policy completa cliccando sul link a fondo pagina."
        },
        "placeholder": {
